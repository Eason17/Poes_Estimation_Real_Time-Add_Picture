...
class PoseEstimator:
    ...
    def solve_pose(self, image_points):
        ...
        # print("旋转向量", rotation_vector, "平移向量", translation_vector)
        return (rotation_vector, translation_vector)  # 数组形式返回旋转向量和平移向量。

    def solve_pose_by_68_points(self, image_points):  # 采用cv2.solvePnP
        """
        Solve pose from all the 68 image points.  求解全部68个图像点的姿态。
        Return (rotation_vector, translation_vector) as pose.  返回(rotation_vector, translation_vector)作为姿态。
        """
    ...
    def draw_annotation_box(self, image, rotation_vector, translation_vector, color=(255, 255, 255), line_width=2):
        ...
        # cv2.projectPoints通过给定的内参数和外参数计算三维点投影到二维图像平面上的坐标。
