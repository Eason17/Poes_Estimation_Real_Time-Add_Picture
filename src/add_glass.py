...
frame = cv2.imread('face_without_glass.jpg')
# frame = cv2.imread('face_right.jpg')
...
for k, d in enumerate(dets):
    ...
    height = p4_y - p3_y
    # print(marks)
    size = (abs(int(width)) + 20, abs(int(height)) - 15)
    ...
    mini_frame = frame[int(p4_y):int(p4_y) + shrink_logo.shape[0],
                 int(p1_x) - 10:int(p1_x) - 10 + shrink_logo.shape[1], :]
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    mask = cv2.inRange(hsv, lower, upper)
    dil = cv2.dilate(mask, kernel, iterations=5)
    mini_dil = np.zeros_like(mini_frame)
    mini_dil[:, :, 0] = dil[int(p4_y):int(p4_y) + shrink_logo.shape[0],
                        int(p1_x) - 10:int(p1_x) - 10 + shrink_logo.shape[1]]
    mini_dil[:, :, 1] = dil[int(p4_y):int(p4_y) + shrink_logo.shape[0],
                        int(p1_x) - 10:int(p1_x) - 10 + shrink_logo.shape[1]]
    mini_dil[:, :, 2] = dil[int(p4_y):int(p4_y) + shrink_logo.shape[0],
                        int(p1_x) - 10:int(p1_x) - 10 + shrink_logo.shape[1]]
    ...
    frame[int(p4_y):int(p4_y) + shrink_logo.shape[0],
    int(p1_x) - 10:int(p1_x) - 10 + shrink_logo.shape[1], :] = mini_frame
    # print(marks[0, 0], '\n', frame.shape, '\n', shrink_logo.shape)
