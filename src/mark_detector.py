...
class MarkDetector:
    ...
    def __init__(self, saved_model='assets/pose_model'):
        ...
        self.graph = tf.Graph()
        config = tf.compat.v1.ConfigProto()  # 为消除warning，替换弃用的tf.ConfigProto
        config.gpu_options.allow_growth = True
        self.sess = tf.compat.v1.Session(graph=self.graph, config=config)  # 为消除warning，替换弃用的tf.Session

        # Restore model from the saved_model file, that is exported by
        # TensorFlow estimator.
        tf.compat.v1.saved_model.loader.load(self.sess, ["serve"], saved_model)  # 没有load方法，版本原因，使用v1版本
